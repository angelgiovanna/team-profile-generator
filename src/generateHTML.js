...
function generateCards(answers) {
    ...
    switch (role) {
        case 'Manager':
            answers.card = 'Office Number: ';
            answers.cardInfo = answers.getOfficeNumber();
            answers.icon = 'fas fa-mug-hot';
            break;
        case 'Engineer':
            answers.card = 'GitHub: ';
            answers.cardInfo = answers.getGithub();
            answers.icon = 'fas fa-glasses';
            break;
        case 'Intern':
            answers.card = 'School: ';
            answers.cardInfo = answers.getSchool();
...
let output = `
    ...
    <div class="card-body">
      ...
      <h5 class="card-text"><i class="${answers.icon} me-2"></i>${answers.getRole()}</h5>
    </div>
    <ul class="list-group list-group-flush">
      <li class="list-group-item">ID: ${answers.id}</li>
      <li class="list-group-item">Email: <a href="mailto:${answers.email}">${answers.email}</a></li>
      <li class="list-group-item">${answers.card}${answers.cardInfo}</li>
    </ul>
  </div>
`;
  return output;
